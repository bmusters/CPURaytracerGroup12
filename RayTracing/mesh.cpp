...
bool Mesh::loadMesh(const char * filename, bool randomizeTriangulation, map<string ,Texture>& textures)
{
    vertices.clear();
    triangles.clear();
	texcoords.clear();
    ...
    {     
        ...
        else if (strncmp(s, "mtllib ", 7)==0)
        {
			...
			loadMtl(file.c_str(), materialIndex, textures);
        }
        // usemtl
...
bool Mesh::loadMtl(const char * filename, std::map<string, unsigned int> & materialIndex, map<string, Texture>& textures)
{
    FILE * _in;
    errno_t err = fopen_s(&_in,filename, "r" );
    if ( err!=0 )
    {
    ...
    while( _in && !feof(_in) )
    {
        ...
        else if (strncmp(line, "map_Kd ",7)==0) // map images
        {
          ...
          // just skip this
			...
			Texture tex = Texture(cimg_library::CImg<float>(t.c_str()));
			textures.insert(make_pair(t, tex));
        }
		else if (strncmp(line, "map_Bump ", 9) == 0) // map images
		{

			std::string n = &(line[9]);
			if (!n.empty() && n[n.length() - 1] == '\n') {
				n.erase(n.length() - 1);
			}
			mat.set_normalName(n);	
			Texture tex = Texture(cimg_library::CImg<float>(n.c_str()));
			textures.insert(make_pair(n, tex));
		}
