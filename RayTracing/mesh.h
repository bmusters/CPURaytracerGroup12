...
class Material
    ...
    public:
		...
		void set_normalName(const std::string & s)//name of the texture image file
		{
			normalName_ = s;
		}
		...
		const std::string & normalName()//name of the normal image file
		{
			return normalName_;
		}
...
public:
    ...
    inline Triangle (const Triangle & t2) {
		...
		materialIndex = t2.materialIndex;
    }
    inline Triangle (unsigned int v0, unsigned int t0, unsigned int v1, unsigned int t1, unsigned int v2, unsigned int t2, int index) {
        v[0] = v0;
        v[1] = v1;
        v[2] = v2;
...
public:
    ...
    void draw();
	bool loadMesh(const char* filename, bool randomizeTriangulation, std::map<std::string, Texture>& textures);
	bool loadMtl(const char* filename, std::map<std::string, unsigned>& materialIndex, std::map<std::string, Texture>& textures);
	void drawSmooth();
